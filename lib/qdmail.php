...
class QdmailBase extends QdmailBranch
{
    ...
    //----------------------------
    public $lang_def			= "ja";
    public $encoding_def		= "utf-8";
    public $detect_def			= array('ASCII','JIS','UTF-8','EUC-JP','SJIS');
    public $mb_parameter_stack = null;
    public $united_charset		= null;
    public $mime_encode_max	= 75;
    //------------------------
    // Time Zone , Message Id
    //------------------------
    public $time_zone			= null; // '+0900' in Japan
    public $message_id			= true;
    public $salt				= 'qdmail';
    public $message_id_right	= null;
    //----------------------------
    // Line Feed Character & kana
    //----------------------------
    public $LFC				=  "\r\n";// Notice: CRLF ,If you failed, change to "\n"
    public $LFC_Qmail			=  null;
    public $is_qmail			=  null;
    public $language			= 'ja';
    public $kana				=  false; // kana header
    //----------
    // sysytem
    //----------
    public $kana_content_relation =  false;
    public $name			= 'Qdmail';
    public $version		= '1.2.6b';
    public $xmailer		= 'PHP-Qdmail';
    public $license 		= 'The_MIT_License';
    //--------------------
    // charset , encoding
    //--------------------
    public $charset_header				= 'iso-2022-jp';
    public $charset_content			= 'iso-2022-jp';
    public $charset_attach_filename	= 'iso-2022-jp';
    public $content_transfer_enc_text	= '7bit';
    public $content_transfer_enc_html	= '7bit';
    public $detect_order				= false;
    //	var $detect_order				= "iso-2022-jp,eucJP-win,UTF-8,SJIS-win,jis,ASCII";
    public $qdmail_system_charset 		= 'utf-8';
    public $force_change_charset		= false;
    public $corres_charset				= array(
    ...
    // for address
    //--------------------------
    public $varidate_address_regex	= '/[^@]+@[^@]+/';
    public $allow_blank_header		= false;
    public $addr_many = array(
        'TO'	=> true,
        'CC'	=> true,
        'BCC'	=> true,
        'FROM'	=> false,
    ...
    public $header_must = array( 'TO' , 'FROM' , 'SUBJECT' );
    public $body_empty_allow = false;
    public $tokey		= array(
        '_ADDR' => 'mail',
        '_NAME' => 'name',
    );
    ...
    // content_id
    //--------------
    public $content_id_include_attach_path = false ;
    public $content_id_only_filename = true ;
    //---------------
    // Once mode
    //---------------
    public $body_structure			= array();
    public $body_build_once		= false;
    public $body_already_build		= false;
    public $attach_build_once		= true;
    public $attach_already_build	= false;
    //------------------------------
    // simple replace
    //------------------------------
    public $simple_replace	= false;
    public $replace		= array();
    public $replace_with_to_priority = true;
    public $replace_def	= array();
    // simple replace command prefix
    public $rep_prefix		= null;
    //---------
    // wordwrap
    //---------
    public $wordwrap_allow	= false;
    public $wrap_prohibit_allow	= false;
    public $wordwrap_length = 45 ;
    // inteligent wordwrap
    // false is that the word exist in the line ,
    // true is that the word must be the beginning of a line
    public $wrap_except	= array(
        'http://' => false,
        'code' => true,
    );
    public $wrap_prohibit_top = ',.;:–?!‼、。．)）]}｝〕〉》」』】〙〗〟’”»ヽヾーァィゥェォッャュョヮヵヶぁぃぅぇぉっゃゅょゎ‐〜？！';
    public $wrap_prohibit_end = '（([{｛〔〈《「『【〘〖‘“« ';
    public $wrap_prohibit = array();
    // multibyte wordwrap , by wordcount or by wordwidth
    public $wrap_width	= true;
    // multibyte wordwidth compare by ascii
    public $mb_strwidth_magni = 2;
    //------------------
    // To Separate mode
    //------------------
    public $to_separate	= false ;
    //----------------------------
    // html mail
    //----------------------------
    public $is_html		= null ;
    public $auto_both		= true ; // text & html
    public $inline_mode	= false;
    public $deco_kind		= null ; // number of $this->deco_def
    public $auto_deco_judge = false;
    public $no_inline_attach_structure = 0;
    public $deco_def_default = 0;
    public $deco_def		= array(
        array(
    ...
    public $allways_bcc	= null ;
    public $header			= array()	;
    public $other_header	= array()	;
    public $header_content_type = array();
    ...
    public $header_for_mailfunction_to;
    public $header_for_mailfunction_subject;
    public $header_for_mailfunction_other;
    public $content_for_mailfunction;
    public $header_for_smtp_array;
    public $content_all_for_smtp;
    public $header_for_smtp;
    //--------------
    // attachament
    //--------------
    public $attach			= array();
    public $attach_path	= null;
    public $auto_ext		= true ; // mimetypes
    public $content_id_fix = false;
    //------------------------
    // Mailer
    //-------------------------
    public $mailer		= 'mail';
    ...
    public $sendmail_path = null;
    //------------------------
    // SMTP
    //-------------------------
    public $smtp				= false ;
    public $smtp_object		= null;
    public $smtp_loglevel_link	= false;
    public $smtp_server = array(
    'host'		=> null ,
    'port'		=> 25 ,
    'from'		=> null,
    'user'		=> null,
    ...
    // render Mode
    //------------------------
    public $render_mode		= false;
    public $size			= array();
    //------------------------
    // Priority
    //------------------------
    public $priority			= null;
    public $priority_def   = array(
    ...
    public $smime						=	false;
    public $pgp						=	false;
    public $private_key_file			= 'private.pem';
    public $certificate_file			= 'cert.pem';
    public $certificate_pass			=  null;
    public $certificate_file_path		=  null;
    public $certificate_temp_path		=  null;
    public $certificate_file_path_win	= 'c:\cert';
    public $certificate_temp_path_win	= 'c:\temp';
    public $certificate_file_path_unix	= '/user/local/cert';
    public $certificate_temp_path_unix	= '/tmp';
    //------------------------
    // etc
    //------------------------
    public $temporary_path		= null;
    public $simple_attach		= false;
    public $keep_parameter		= array(false);
    public $mta_option			= null ;
    public $is_create			= false;
    public $address_validation_method  = array('this','validateAddr');
    public $boundary_base_degit = 2 ;
    public $stack_construct	= null ;
    ...
    //------------------------------
    public $log_level		= 0 ;
    public $log_level_max  = 3 ;
    public $log_path  		= './';
    public $log_filename	= 'qdmail.log';
    public $log_append		= 'a' ;
    ...
    public $errorlog_level_max = 3 ;
    public $errorlog_path  = './';
    public $errorlog_filename = 'qbmail_error.log';
    ...
    //----------------
    public $debug			= 0 ;
    public $debug_report	= false;
    public $debug_report_path = './';
    public $debug_echo_charset = true;
    ...
    //--------------------------
    public function makeDecoTemplate($deco_kind, $content)
    {
    ...
    // Easy Base
    //-------------------
    public function easy($type, $to, $subject, $content, $other_header = array(), $attach = null)
    {
    ...
    public function easyText($to, $subject, $content, $other_header = array(), $attach = null, $option = array())
    {
        return $this->easy(array('TYPE' => 'text','OPTION' => $option), $to, $subject, $content, $other_header, $attach);
    }

    public function easyHtml($to, $subject, $content, $other_header = array(), $attach = null, $option = array())
    {
        return $this->easy(array('TYPE' => 'html','OPTION' => $option), $to, $subject, $content, $other_header, $attach);
    }

    public function easyReplace($to, $subject, $content, $other_header = array(), $attach = null, $option = array())
    {
    ...
    public function easyDeco($to, $subject, $content, $other_header = array(), $attach = null, $option = array())
    {
        if (isset($attach)) {
    ...
    public function easyDecoTemplate($to, $subject, $template, $other_header = array(), $attach = null, $option = array())
    {
        if (is_null($attach)) {
            $attach = array();
        }
    ...
    public function easyDecoRep($to, $subject, $content, $other_header = array(), $attach = null, $option = array())
    {
        $this->simpleReplace(true);
        return $this->easyDeco($to, $subject, $content, $attach, $option, $option);
    }

    public function easyOption($to, $subject = null, $content = null, $other_header = array(), $attach = null, $option = array())
    {
    ...
    */
    ...
    public $method_property	= array();

    public function optionNameLink()
    {
    ...
    public function option($option, $line = null, $min = null, $max = null)
    {
        $ret = array();
        if (!is_null($line)) {
            $line = '-' . $line ;
    ...
    public function autoBoth($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function toSeparate($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function simpleReplace($bool = null)
    {
        $this->toSeparate($bool);
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function autoDecoJudge($bool = null)
    {
        $this->attachBuildOnce(!$bool);
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function autoExt($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function bodyEmptyAllow($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function ignoreError($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function forceChangeCharset($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function errorDisplay($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function sendmail($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function smtp($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function smtpLoglevelLink($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function inlineMode($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function replaceWithToPriority($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function attachBuildOnce($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function bodyBuildOnce($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function kana($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function keepParameter($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function renderMode($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function simpleAttach($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function messageId($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function allowBlankHeader($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function smime($bool = null)
    {
    ...
    public function size($kind = null)
    {
    ...
    public function sizeBody()
    {
        return $this->size('BODY');
    }
    public function smtpData()
    {
        if (empty($this->header_for_smtp)) {
            $stack = $this->render_mode;
            $this->render_mode = true;
    ...
    public function lineFeed($LFC = null)
    {
        if (is_null($LFC)) {
            return $this->LFC;
        }
    ...
    // something change mode
    //---------------------------------------
    public function whichTextHtml($which)
    {
    ...
    public function allwaysBcc($option = null)
    {
        if (is_null($option)) {
    ...
    public function priority($option = null)
    {
    ...
    public function certificatePass($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function certificateFilePath($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function certificateTempPath($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function certificateFile($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function privateKeyFile($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function framework($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function attachPath($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function timeZone($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function messageIdRight($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function mtaOption($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function unitedCharset($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function varidateAddressRegex($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function sendmailPath($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function temporaryPath($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function logPath($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function errorlogPath($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function logDateFormat($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function logFilename($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function errorlogFilename($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function logLevel($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__, 0, $this->log_level_max);
    }
    public function errorlogLevel($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__, 0, $this->errorlog_level_max);
    }
    public function mimeEncodeMax($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__);
    }
    public function smtpServer($array = null)
    {
        return $this->option(array( __FUNCTION__ => $array ), __LINE__, true, true);
    }
    public function addressValidationMethod($array = null)
    {
    ...
    //mb_ wrapper
    //------------------
    public function qd_detect_encoding($word)
    {
    ...
    public function qd_convert_encoding($word, $target_chrset, $org_charset = null)
    {
    ...
    public function qd_detect_order($param = null)
    {
        if (is_null($param)) {
    ...
    //-----------------------------------------
    public function addressField($addr = null, $name = null)
    {
    ...
    //-----------------------------------------------------------
    public function wordwrapProhibitConstruct()
    {
    ...
    public function strToArrayKey($word, $value)
    {
        $ret = array();
    ...
    public function wordwrapAllow($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function wrapProhibitAllow($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function wrapProhibitEnd($option = null)
    {
        $this->option(array( __FUNCTION__ => $option ), __LINE__);
        $this->wordwrapProhibitConstruct();
        return  $this->errorGather();
    }
    public function wrapProhibitTop($option = null)
    {
        $this->option(array( __FUNCTION__ => $option ), __LINE__);
        $this->wordwrapProhibitConstruct();
        return  $this->errorGather();
    }
    public function wrapWidth($bool = null)
    {
        return $this->option(array( __FUNCTION__ => $bool ), __LINE__);
    }
    public function wordwrapLength($length = null)
    {
        if (is_null($length)) {
    ...
    public function mbStrwidthMagni($option = null)
    {
        return $this->option(array( __FUNCTION__ => $option ), __LINE__, 0, 10);
    }
    public function wrapExcept($array = null)
    {
        if (null === $array) {
    ...
    // Charset Option
    //------------------------------------------
    public function charsetHeader($charset = null)
    {
        if (is_null($charset)) {
    ...
    public function charsetAttach($charset = null)
    {
        if (is_null($charset)) {
    ...
    public function charsetBody($charset = null, $enc = null)
    {
    ...
    public function charset($array = null, $op = null)
    {
    ...
    public function encoding($enc = null)
    {
        if (is_null($enc)) {
    ...
    //--------------------------
    public function setMbParameter($lang = null, $internal_enc = null, $detect = null)
    {
    ...
    public function decoDef($value = null)
    {
        if (is_null($value)) {
    ...
    public function decoFix($cari = null)
    {
        if (is_null($cari)) {
    ...
    public function decoSelect($deco_kind = null)
    {
    ...
    public function decoJudge($addr_array)
    {
    ...
    //------------------------------------
    public function replaceWord($array = null, $add = false)
    {
        if (is_null($array)) {
            return $this->replace ;
        }
    ...
    public function replaceDef($array = null)
    {
        if (is_null($array)) {
    ...
    public function repPrefix($option = null)
    {
        return $this->stringOption(__FUNCTION__, $option, __LINE__);
    }
    public function replace($cont, $rep)
    {
    ...
    public function qdmail_preg_replace($reg, $rep, $cont)
    {
    ...
    public function replyto($addr = null, $name = null)
    {
        return $this->addrs('REPLYTO', $addr, $name, false);
    }
    public function addHeader($header_name = null, $value = null)
    {
    ...
    public function reset($debugErase = false)
    {
    ...
    public function resetHeaderBody()
    {
        $this->resetBody();
        $this->resetHeader();
    }

    public function _gatherFromArray($array, $key)
    {
        $ret = array();
        foreach ($array as $ar) {
            $ret[] = $ar[$key] ;
    ...
    public function body($type = null, $cont = null, $length = null, $charset = null, $enc = null, $org_charset = null)
    {
        if (is_null($type)) {
            return $this->content;
        }
    ...
    public function html($cont, $charset = null, $enc = null, $org_charset = null)
    {
    ...
    //--------------------------
    public function addrs($section, $addr = null, $name = null, $add = false)
    {
    ...
    public function analyzeAddr($addr, $name)
    {
        if (is_string($addr)) {
            if (empty($name)) {
    ...
    //--------------------------------------------------------
    public function _extractNameAddr($addr)
    {
    ...
    public function setContentArgs($type, $param)
    {
        $method_name = 'text';
        if ('HTML' == $type) {
            $method_name = 'html';
    ...
    public function selectReplace($to, $key)
    {
        $ret = array();
    ...
    //  Create one mail
    //-----------------------------------------------
    public function createMail($boundary = null, $boundary_fix = false)
    {
    ...
    public function replaceBodyStructure($kind)
    {
    ...
    public function & serchBodyStructure($content_type, & $bbs, & $false)
    {
    ...
    //	)
    //-------------------------
    public function isInlineImage($filename)
    {
    ...
    public function buildBody($structure, $boundary = null, $rel = false, $boundary_fix = false)
    {
        $ret = array();
        $one = array();
        if (is_null($boundary)) {
    ...
    public function expandHeader($hds)
    {
        if (empty($hds)) {
            return null;
        }
    ...
    public function makeContentText($content, $is_text = 'TEXT')
    {
    ...
    public function mimeEncode($subject, $charset, $org_charset = null, $first_line_front_words_length = 12)
    {
    ...
    public function extractAddr($addr_including_sclub)
    {
    ...
    //  (Overload)
    //----------------------------------------------------------------
    public function _charsetDefFix()
    {
        if (!isset($this->deco_kind)) {
            return ;
        }
    ...
    //------------------------------------------------------------
    public function setAddr($header)
    {
    ...
    public function convertCharsetRecursive($array, $target_enc)
    {
    ...
    public function extractrecipient()
    {
        $hd = array('to','cc','bcc') ;
        $ret = array();
        foreach ($hd as $hdn) {
    ...
    //--------------------------------------------------------
    public function buildAttach($one, $boundary, $inline)
    {
        $ret_boundary = null;
        $ret_header = array();
        $ret_content = null;
    ...
    public function isSetCid($array)
    {
        return  isset($array['CONTENT-ID']) && ('' !== $array['CONTENT-ID']) ;
    }
    public function makeContentId($id)
    {
    ...
    public function replaceCid($content)
    {
        if (!isset($this->deco_kind)) {
            return $content;
        }
    ...
    public function attachDirect($attach_name, $data, $add = false, $mime_type = null, $content_id = null, $target_charset = null, $charset_org = null)
    {
        $_att = array();
        $_att[0]['DIRECT'] = true;
        $_att[0]['DATA'] = $data;
    ...
    public function attachPathFix($path_filename)
    {
    ...
    //
    //---------------------------------------
    public function mbWordwrap($word, $length)
    {
    ...
    public function mbWordwrapLine($line, $length)
    {
        $skip = false;
    ...
    public function defMbSubStr($line, $start, $length, $enc)
    {
        return array( mb_substr($line, $start, $length, $enc) ,$length );
    }
    public function widthSubStr($line, $start, $length, $enc)
    {
        $ret = array();
    ...
    public function mbProhibitTop($line, $enc)
    {
        $flag = false;
        $ret = null;
        $len = mb_strlen($line, $enc);
    ...
    public function mbProhibitEnd($line, $enc)
    {
        $flag = false;
        $len = mb_strlen($line, $enc);
        $count = 0 ;
    ...
    // utility
    //------------------------
    public function issetContent($array)
    {
        if (!isset($array)) {
            return false ;
        }
    ...
    public function keyUpper($array)
    {
    ...
    public function qdmail_array_combine($keys, $values) //for php4
    ...
    public function qdmail_md($col = null, $start = 33, $end = 126)
    {
        if (is_null($col)) {
    ...
    public function quotedPrintableEncode($word)
    {
        if (empty($word)) {
            return $word;
        }
    ...
    // error
    //------------------
    public function errorStatment($type = false, $lf = null)
    {
        if ($type) {
    ...
    public function errorSpecify($func, $line, $add_message = null)
    {
    ...
    public function debugEchoLine()
    {
        $vars = func_get_args();
        $this->debugEcho(false, $vars);
    }
    public function debugEchoLf()
    {
        $vars = func_get_args();
        $this->debugEcho(true, $vars);
    }
    public function debugEcho($lf, $vars = null)
    {
    ...
    public function debugReport($var = null)
    {
        if (is_null($var)) {
...
EOF;
    ...
    //--
    public function debugReportPath($path = null)
    {
        if (is_null($path)) {
            return $path;
        }
    ...
    public function sendBySmtp($obj = null)
    {
        if (!is_null($obj)) {
    ...
    public function sendBySendmail()
    {
    ...
    //------------------------------------------
    public function & smtpObject($null = false)
    {
        if (is_null($null)) {
    ...
    public function setSmtpObject(& $obj)
    {
        if (is_object($obj)) {
            $this->smtp_object = & $obj;
            return true;
...
class QdmailUserFunc extends QdmailBase
{
    ...
    public function validateAddr($addr)
    {
    ...
    public function stripCrlf($word)
    {
...
class Qdmail extends QdmailUserFunc
{
    ...
    public function Qdmail($param = null)
    {
        if (!is_null($param)) {
            $param = func_get_args();
        }
...
class QdmailComponent extends QdmailUserFunc
{
    ...
    public function QdmailComponent($param = null)
    {
        if (!is_null($param)) {
            $param = func_get_args();
        }
    ...
    // Cake Interface
    //----------------------------
    public function import($kind, $name)
    {
    ...
    public function cakeText($content, $template = null, $layout = null, $org_charset = null, $target_charset = null, $enc = null, $wordwrap_length = null)
    {
    ...
    public function cakeHtml($content, $template = null, $layout = null, $org_charset = null, $target_charset = null, $enc = null)
    {
    ...
    public function cakeRender($content, $type, $org_charset = null, $target_charset = null)
    {
...
class sfQdmail extends QdmailUserFunc
{
    ...
    public function addStringAttachment($data, $attach_name, $mime_type)
    {
